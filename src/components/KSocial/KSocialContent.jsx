...
const SocialCard = ({
	...
	return (
      ...
      marginTop:10,
			...
			<Image
				style={{ width: 375, height: 302, marginTop: 15, marginLeft: 0 }}
				source={{ uri: item?.contentUrl ? item?.contentUrl[0] : "" }}
			/>
			<View style={{ flexDirection: "row", display: "flex" }}>
				...
				<Entypo
					style={{ marginLeft: 160, marginTop: 7 }}
					name="share"
					size={24}
					color="black"
				/>
...
const BuyerKSocial = ({ navigation }) => {
	...
	const uploadStatus = async (assets) => {
		...
		try {
			...
			console.error(error);
			setFailedModalVisible(true);
			// Alert.alert("Error", "An error occureddedededede ");
		} finally {
			// setIsLoading(false);
		}
	};
		...
		>
			...
			{showGallery && (
				...
				>
					<ImagePicker
						onSave={(assets) => {
							/*setFile(assets);*/ uploadStatus(assets);
							console.log(assets);
			...
			{postData && (
				<View>
					...
					<MyModal
						...
						text={"An error occured while loading status"}
						button={"Try again"}
						ButtonColor={"#EB270B"}
					/>
				</View>
