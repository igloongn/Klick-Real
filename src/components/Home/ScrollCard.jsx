...
const ScrollCard = ({ item, navigation }) => {
	...
	return (
		<TouchableOpacity
			onPress={() =>
				navigation.navigate({ name: "shoppage", params: { id: item.id } })
			}
			style={styles.sponsoredcard}
		>
			<Image style={styles.sponsoredimage} source={{ uri: item?.logo }} />
			<View style={{ flexDirection: "column", display: "flex" }}>
				<Text style={styles.sponsorbigtext} numberOfLines={1}>
					{item?.name}
				</Text>
